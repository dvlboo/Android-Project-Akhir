...
class CountDownTableActivity : AppCompatActivity() {
    ...
    private var isQRCodeValid: Boolean = false
...
//            codeScan.isFlashEnabled = !codeScan.isFlashEnabled
            ...
            decodeCallback = DecodeCallback {
                runOnUiThread {
                    ...
                    val elapsedTime = currentTime - lastScanTime

                    if (isScanningEnabled && elapsedTime >= 5000) {
                        lastScanTime = currentTime
                        hasilscan = it.text
                        if (hasilscan == "1000$meja" && !isQRCodeValid) {
                            isQRCodeValid = true
                            pindah()
                        } else {
                            showToast("QR-CODE Wrong!")
                        }
                    }
                }
            }
    ...
    private fun pindah() {
        ...
        val documentReference2: DocumentReference = store.collection("users").document(userId)
        documentReference2.set(newData2, SetOptions.merge())

        showToast("Managed to book a table")
        intent = Intent(this@CountDownTableActivity, MenuActivity::class.java)
...
//            startActivity(Intent(this, TableActivity::class.java))
            ...
            showToast("Back again to cancel table reservation")
