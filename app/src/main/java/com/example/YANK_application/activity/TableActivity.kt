...
//            val A5 = binding.button5.text.toString().trim()
        ...
        binding.buttonB5.setOnClickListener{
            updateDataMeja("B5")
        }
    ...
    override fun onStart() {
        ...
        val buttonList = listOf(
            ...
            binding.button8, binding.button9, binding.button10, binding.buttonB1, binding.buttonB2, binding.buttonB3, binding.buttonB4,
            binding.buttonB5, binding.buttonC1, binding.buttonC2, binding.buttonC3
        )
        val documentReference: DocumentReference = store.collection("meja").document("reservasimeja")
...
// ...
                fieldMap?.forEach { (key, value) ->
                    ...
                    // Tambahan untuk baris B
                    if (key.startsWith("B")) {
                        val button = buttonList[(10 + key.substring(1).toIntOrNull()?.minus(1)!!)
                            ?: 0]
                        if (value == "Di pesan") {
                            button.isEnabled = false
                            button.backgroundTintList = ContextCompat.getColorStateList(this, R.color.grey)
                    ...
                    // Tambahan untuk baris C
                    if (key.startsWith("C")) {
                        val button = buttonList[(15 + key.substring(1).toIntOrNull()?.minus(1)!!)
                            ?: 0]
                        if (value == "Di pesan") {
                            button.isEnabled = false
                            button.backgroundTintList = ContextCompat.getColorStateList(this, R.color.grey)
